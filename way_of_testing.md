...
### V0版 象征性测试了最常见的场景
不少程序员看到这个题目，第一时间就兴高采烈地随意写了两个测试用例，心想：不就是排除最小值嘛。这代表了一部分公司的测试现状，表现出的问题是：
...
### V1版 考虑特殊情况下的用例
若拿V0版做测试用例评审，很快就会发现没考虑零和负数的情况，也没考虑传入参数不合法的情况。像阿里巴巴这样的公司通常会有一个测试用例评审的流程。
...
与V1版大部分相同，区别只在：正常流数据测试素材多了4种（也就是多了4个测试用例），达到了8种。
...
使用V2版的测试用例去测试V1版的实现代码，会发现有测试用例无法通过，是因为V1版没有考虑正数的数量（正数在乘法里没有特殊性质）。
